...
#这个不是国家地块颜色，作者也不知道这里的color是做什么用，建议使用和国家地块颜色相同的RGB代码{国家地块颜色在common\\countries\\colors.txt改}'''
    ...
    with open(country_tag_file_path, "w", encoding="utf-8") as f:
        f.write(f'{country_tag} = "countries/{country_name}.txt"  “请将内容复制到common-country-tags-00_countries.txt”')

    history_file_path = os.path.join(history_dir, "countries", f"{country_tag} - {country_name}.txt")
    with open(history_file_path, "w", encoding="utf-8") as f:
        f.write(
'''capital = 
#首都地块province(带#的均为注释，无效果，若想要效果，就删除#)
...
set_politics = {
	...
	last_election = 
	#最后一次选举（"年.月.日"  如  "1936.1.1"（p社sb代码，无法早于该日期））
...
#不要动
	...
	#不要动	
		...
		#国家精神名字
			...
			name = 国家tag_精神名字，如USA_123
			#国家精神名字

			picture = 
            #国家精神图片（p社目前的主流是将其注册在\\interface中的ideas.gfx文件中后，再将其name贴入这里。其构成简单，你们可以自行去那个.gfx文件中学习）
			allowed = { original_tag =  }
			#允许哪个国家使用，original_tag = XXX 是原始tag为XXX的国家可以使用该民族精神。想让所有国家都可以使用该民族精神的写法为always = yes
